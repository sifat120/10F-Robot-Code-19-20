 ...
 * @param itheta QAngle
 */ 
odomDebug::state_t::state_t(okapi::QLength ix, okapi::QLength iy, okapi::QAngle itheta) : x(ix), y(iy), theta(itheta) {}
 ...
 */ 
odomDebug::state_t::state_t(double ix, double iy, double itheta) : x(ix * okapi::inch), y(iy * okapi::inch), theta(itheta * okapi::radian) {}
...
void odomDebug::setData(state_t state, sensors_t sensors) {
  ...
  double c_x = state.x.convert(okapi::court);
  double c_y = (1_crt - state.y).convert(okapi::court);
  double c_theta = state.theta.convert(okapi::radian);
  ...
  std::string text =
  "X_in: " + std::to_string(state.x.convert(okapi::inch)) + "\n" +
  "Y_in: " + std::to_string(state.y.convert(okapi::inch)) + "\n" +
  "Theta_deg: " + std::to_string(state.theta.convert(okapi::degree)) + "\n" +
...
lv_res_t odomDebug::tileAction(lv_obj_t* tileObj) {
  ...
  int y = num / 6;
  int x = num - y * 6;
  if(that->stateFnc) that->stateFnc({x * okapi::tile + 0.5_tl, 1_crt - y * okapi::tile - okapi::0.5_tl, okapi::0_deg});
